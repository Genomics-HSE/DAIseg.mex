...
def createDataFrameMex(seq_length,rr,mu,cut,Lmbd_opt, n_st, seq, n_neanderthal,  
    ...
    for idx in idx_mas:
        ...
        real_tracts = colored_intervals(nd_tracts[idx], eu_tracts[idx], na_tracts[idx], af_tracts[idx])

        cl_report = classification_rpt(confusion_mtrx(real_tracts, tracts_HMM))

#        for l in intersections(na_tracts[idx], nd_tracts[idx]):
#            print(l)

#        for l in  tracts_HMM[3]:
#            print(l)
