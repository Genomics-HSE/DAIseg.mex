...
def EM2_gaps(seq, lambda_0,n_st):
    return EM.EM_common_gaps(P, seq, n_st, MU, RR, Lambda_0, epsilon, L, bnds, args.EM_steps, gaps_numbers)
...
with open(args.o_eu, "w") as f:
   for idx in range(0, len(seq)): 
       f.write(str(usfl.intersections(tracts_HMM_mas[idx][1], domain))+'\n') 
       
with open(args.o_na, "w") as f:
   for idx in range(0, len(seq)):
       f.write(str(usfl.intersections(tracts_HMM_mas[idx][3], domain))+'\n') 
